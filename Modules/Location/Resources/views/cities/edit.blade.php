...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">@lang('Edit City')</h1>
</div>
<div class="row">
    <div class="col-md-12">

        <form role="form" method="post" action="{{ route('settings.location.cities.update', ['id' => $item->id]) }}">
            @csrf
            @method('PUT')
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                                ...
                                <select name="country_id" class="form-control">
                                    @foreach($countries as $country)
                                        <option value="{{ $country->id }}" {{ old('country_id', $item->state->country_id) == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                                    @endforeach
                                </select>
                                @error('country_id')
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="name" value="{{ old('name', $item->name) }}" class="form-control" placeholder="@lang('City')">
                                @error('name')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
...
@push('scripts')
<script>
    const DEFAULT_STATE = "{{ old('state_id', $item->state_id) }}";

                ...
                <div class="container">
                    <div class="row mt-3">
                        <div class="col-md-12">
                            <div class="row mb-2">
                                <div class="col-md-12">
                                    <h3 class="text-white"><strong>@lang('Discover now all company on') {{ config('app.name') }}</strong></h3>
                                </div>
                            </div>
                            <form id="form_companies_search" action="javascript:void(0);">
                                <div class="row bd-search-box">
                                    <div class="form-group col-md-4">
                                        <input class="form-control" value="" placeholder="@lang('Company title, keywords...')"
                                        ...
                                        <select class="form-control" id="city" tabindex="-1" aria-hidden="true">
                                            ...
                                            @foreach ($cities as $city)
                                                <option value="{{ $city->id }}"
                                                    {{ $filter_city_id == $city->id ? 'selected' : '' }}>{{ $city->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <select class="form-control" id="industry" tabindex="-1" aria-hidden="true">
                                            ...
                                            @foreach ($industries as $industry)
                                                <option value="{{ $industry->id }}"
                                                    {{ $filter_industry_id == $industry->id ? 'selected' : '' }}>

            ...
            <div class="row mt-4">
                <div class="col-sm-8">
                    ...
                    <a href="{{ url('templates') }}"
                        class="btn @if (empty($route_id)) btn-secondary @else btn-light @endif btn-sm">@lang('All Templates')</a>
                    @foreach ($categories as $category)
                        <div class="btn-group" role="group">
                            <a class="btn @if ($route_id == $category->id) btn-secondary @else btn-light @endif btn-sm"
                                href="{{ url('templates/') . '/' . $category->id }}">{{ $category->name }}</a>
                        </div>
                    @endforeach
                </div>
            </div>
                        ...
                        <div class="col-md-4 itembb">
                            <div class="clearfix blog-bottom blog blogitemlarge">
                                <a href="#" data-id="{{ $item->id }}" data-toggle="modal"
                                    data-target="#createModal" title="{{ $item->name }}"
                                    ...
                                    @if ($item->is_premium)
                                        <span class="post-date badge badge-danger"><i class="fas fa-star"></i>
                                            @lang('Premium')</span>
                                    @else
                                        <span class="post-date badge badge-success"><i class="fas fa-star"></i>
                                            @lang('Free')</span>
                                    @endif
                                </a>
                                <div class="content_blog clearfixflex flex-column flex-lg-row">
                                    ...
                                    <div class="d-flex pt-1">
                                        <a href="#" class="btn btn-sm btn-primary mr-2 btn_builder_template"
                                            data-id="{{ $item->id }}" data-toggle="modal"
                                            data-target="#createModal">@lang('Builder')</a>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <form id="" action="{{ route('resumecv.save') }}" method="post"
                            enctype='multipart/form-data'>
                            @csrf
                            <div class="modal-body">
                                <div class="form-group">
                                    <input type="number" class="form-control" name="template_id" hidden=""
                                        required="" id="template_id_builder">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-primary btn-sm"
                                    id="saveandbuilder">@lang('Save & Builder')</button>
                            </div>
                        </form>
                    </div>
                </div>

...
@section('title', $translates['Names'])
@section('content')

<div class="d-sm-flex align-items-center justify-content-between mb-4">
      <div>
        <a href="{{ route('settings.job.attributes.' . $attr_route . '.create') }}" class="btn btn-primary">{{ $translates['title_create'] }}</a>
      </div>
                    ...
                    <table class="table card-table table-vcenter text-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('settings.job.attributes.' . $attr_route . '.edit', ['id' => $item->id]) }}" class="btn btn-primary">@lang('Edit')</a>
                                    <form class="d-inline-block form-delete" action="{{ route('settings.job.attributes.' . $attr_route . '.destroy', ['id' => $item->id]) }}" method="post">
                                        @csrf
                                        @method('DELETE')

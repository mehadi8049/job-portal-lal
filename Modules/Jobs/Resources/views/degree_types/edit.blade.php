...
@section('title', __('Edit Degree type'))

@section('content')

<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">@lang('Edit Degree type')</h1>
</div>
<div class="row">
    <div class="col-md-12">

        <form role="form" method="post" action="{{ route('settings.job.attributes.degree_types.update', ['id' => $item->id]) }}">
            @csrf
            @method('PUT')
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="name" value="{{ old('name', $item->name) }}" class="form-control" placeholder="@lang('Degree type')">
                                @error('name')
                            ...
                            <div class="form-group">
                                ...
                                <select name="degree_level_id" class="form-control">
                                    @foreach($degree_levels as $degree_level)
                                        <option value="{{ $degree_level->id }}" {{ old('degree_level_id', $item->degree_level_id) == $degree_level->id ? 'selected' : '' }}>{{ $degree_level->name }}</option>
                                    @endforeach
                                </select>

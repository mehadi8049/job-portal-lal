                        ...
                        <tbody>            
                            ...
                            <tr>
                               ...
                               <td>
                                @if(isset($item->job))
                                <small>@lang('Job Tile'): 
                                  <a href="{{ route('settings.jobs.edit', ['job' => $item->id]) }}" class="text-dark">
                               ...
                               <td>
                                  @if(isset($item->job->company))
                                   <small>@lang('Name'): <a href="{{ route('settings.companies.edit', ['company' => $item->job->company]) }}" class="text-dark">
                                    <strong>{{ $item->job->company->company_name }}</strong>
                                    </a>
                                   </small><br>
                                   <small>@lang('Email'): <strong>{{ $item->job->company->company_email }}</strong></small>
                                   @endif
                               </td>
                               <td>
                                 <small>@lang('Full name'): <strong>{{$item->fullname}}</strong></small><br>
                                 <small>@lang('Email'): <strong>{{$item->email}}</strong></small>
                               </td>
                               ...
                               <small>{{$item->description}}</small>
                                ...
                                <td>
                                  <div class="d-flex">
                                        ...
                                        <div class="p-1"> 
                                          <form method="post" action="{{ route('settings.jobs.applicants.destroy', $item->id) }}" onsubmit="return confirm('@lang('Confirm delete?')');">
                                              @csrf
                                              @method('DELETE')
